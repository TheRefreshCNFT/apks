...
PROJECT_DIR="/home/user/apks/Chompy"
SRC_DIR="$PROJECT_DIR/app/src/main"
BUILD_DIR="$PROJECT_DIR/build"
ANDROID_JAR="/usr/lib/android-sdk/platforms/android-23/android.jar"

echo "=== CHOMPY APK BUILD ==="
...
javac -source 8 -target 8 \
  ...
  -d "$BUILD_DIR/obj" \
  "$BUILD_DIR/gen/com/chompy/game/R.java" \
  "$SRC_DIR/java/com/chompy/game/MainActivity.java" \
  2>&1
...
aapt package -f \
  ...
  -M "$SRC_DIR/AndroidManifest.xml" \
  -I "$ANDROID_JAR" \
  -F "$BUILD_DIR/chompy-unsigned.apk" \
  -A "$SRC_DIR/assets"

# Step 5: Add DEX to APK
echo "[5/6] Adding DEX to APK..."
cd "$BUILD_DIR"
aapt add -f "$BUILD_DIR/chompy-unsigned.apk" classes.dex
...
jarsigner -verbose \
  ...
  -storepass android \
  -keypass android \
  "$BUILD_DIR/chompy-unsigned.apk" \
  androiddebugkey

# Copy final APK
cp "$BUILD_DIR/chompy-unsigned.apk" "$PROJECT_DIR/chompy.apk"

echo ""
echo "=== BUILD COMPLETE ==="
echo "APK: $PROJECT_DIR/chompy.apk"
echo "Size: $(du -h "$PROJECT_DIR/chompy.apk" | cut -f1)"
